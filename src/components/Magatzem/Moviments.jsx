...
function Moviments() {
  ...
  useEffect(() => {
    const canvimoviments = async () => {
      try {
        const apiUrl = import.meta.env.VITE_API_URL;

        axios.get(`${apiUrl}/Moviment`, {headers: {"auth-token": localStorage.getItem("token")}})
        .then(response => {
        ...
        axios.get(`${apiUrl}/User`, {headers: {"auth-token": localStorage.getItem("token")}})
        .then(response => {
        ...
        const responseProduct = await axios.get("http://node.daw.iesevalorpego.es:3001/Product");
            ...
            {moviments.length === 0 ? (
              ...
              moviments.map((valors) => (
                <tr key={valors.id}>
                  ...
                  <td className='d-block d-md-table-cell'>
                  <p className="d-sm-inline d-md-none">ID producto:</p> {valors.product_id}
                  </td>
                  <td className='d-block d-md-table-cell'>
                  ...
                  <td className='d-block d-md-table-cell'>
                  <p className="d-sm-inline d-md-none">Operari:</p> {usuari(valors.operator_id)}</td>
                  <td className='d-block d-md-table-cell'>
                  <p className="d-sm-inline d-md-none">Origen:</p> {valors.orgin}</td>
                  <td>
          ...
          <Modal.Body>
            {selectMoviments ? (
              <div>
                ...
                <p><b>Operari:</b> {usuari(selectMoviments.operator_id)}</p>
