...
useEffect(() => {
  const agafarmoviments = async () => {
    try {
      ...
      const response2 = await axios.get("http://node.daw.iesevalorpego.es:3001/Product");
      const response3 = await axios.get("http://node.daw.iesevalorpego.es:3001/User");
...
function Moviments() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const responseMoviments = await axios.get("http://node.daw.iesevalorpego.es:3001/Moviment");
        const responseUsers = await axios.get("http://node.daw.iesevalorpego.es:3001/User");

        setMoviments(responseMoviments.data);
  ...
  return (
    <>
      <div>
        <Header title="Moviments" />
        <Filtres />
        <div className="table-responsive" style={{ marginTop: 70 }}></div>
        <table className='table table-striped'>
          <thead>
            <tr>
              <th className="d-none d-md-table-cell">ID</th>
              ...
              moviments.map((valors) => (
                <tr key={valors.id}>
                  ...
                  <td className='d-block d-md-table-cell'>
                    ...
                    {valors.product_id}</td>
                  <td className='d-block d-md-table-cell'>{valors.storage_id}</td>
                  <td className='d-block d-md-table-cell'>{valors.street_id}</td>
                  <td className='d-block d-md-table-cell'>{valors.shelf_id}</td>
                  <td className='d-block d-md-table-cell'>{valors.space_id}</td>
                  <td className='d-block d-md-table-cell'>{valors.quantity}</td>
                  <td className='d-block d-md-table-cell'>{valors.date}</td>
                  <td className='d-block d-md-table-cell'>{getUserName(valors.operator_id)}</td>
                  <td className='d-block d-md-table-cell'>{valors.orgin}</td>
                  <td>
                    <Button variant="outline-secondary" onClick={() => handleShow(valors)}>
                      <i className="bi bi-eye p-2"></i>
                    </Button>
                  </td>
                </tr>
          ...
          <Modal.Body>
            {selectMoviments ? (
              <div>
                ...
                <p><b>Quantitat:</b> {selectMoviments.quantity}</p>
                <p><b>Data:</b> {selectMoviments.date}</p>
                <p><b>Operari:</b> {getUserName(selectMoviments.operator_id)}</p>
                <p><b>Origen:</b> {selectMoviments.orgin}</p>
              </div>
            ) : (
              <p>No hay detalles disponibles.</p>
